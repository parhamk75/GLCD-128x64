...
HAL_StatusTypeDef GLCD_L2_SetWholeDispColor(GLCD_L0_TypeDef* pglcd_, GLCD_L2_DispColor_TypeDef init_whole_dsp_clr_, GLCD_L2_HALF_TypeDef hlf_)
{
    ...
    {
        GLCD_L2_GotoX(pglcd_, i, GLCD_L2_HALF_Both);
        for (uint8_t j = 0; j <= GLCD_L0_Y_MAX; j++)
        {
            HAL_StatusTypeDef tmp_state = GLCD_L2_WriteByte(pglcd_, (uint8_t)init_whole_dsp_clr_* 0xFF, hlf_);
            if( tmp_state != HAL_OK )
            {
                if( tmp_state == HAL_BUSY )
                {
                    while(GLCD_L2_WriteByte(pglcd_, (uint8_t)init_whole_dsp_clr_* 0xFF, hlf_) != HAL_OK );
                }
            }
        }
    }
